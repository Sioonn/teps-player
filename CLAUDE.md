...
- **Script toggle/highlight**: 문항 선택 시 스크립트 본문은 기본 숨김. `스크립트 보기`로 펼치고 문항이 바뀌면 다시 숨김. 본문 텍스트를 드래그하면 `강조하기` 버튼을 표시. 강조는 즉시 화면과 localStorage에 저장하고 Supabase는 가능할 때 백그라운드 동기화
